...
class _BookmarksViewState extends State<_BookmarksView> {
  Timer? _loadBookmarksTimer;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadBookmarksTimer = Timer(const Duration(milliseconds: 120), () {
        if (!mounted) return;
    ...
    return PremiumScaffold(
      child: BlocBuilder<BookmarksCubit, BaseState<List<JobModel>>>(
        builder: (context, state) => BlocStateWidget<List<JobModel>>(
          ...
          emptySubtitle:
              'Bookmark jobs you want to revisit and they will appear here.',
            ...
            itemBuilder: (_, index) {
              if (index == 0) {
                return PageHeader(
                  ...
                  subtitle:
                      'A cleaner shortlist of roles you may want to return to.',
                  actions: [
                ...
                child: Dismissible(
                  ...
                  onDismissed: (_) {
                    ...
                    AppSnackBars.showInfo(
                      ...
                      actionLabel: 'Undo',
                      onAction: () =>
                          context.read<BookmarksCubit>().addBookmark(job.id),
                    );
                  },
                  child: AppCard(
                    shadowLevel: AppCardShadowLevel.none,
